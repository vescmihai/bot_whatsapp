...
def test_simple():
    ...
    try:
        # Paso 1: Verificar conexión BD
        print("1️⃣ Verificando conexión a BD...")
        gestor = GestorBaseConocimientoFixed()
        ...
        if test_query and len(test_query) > 0:
            ...
            print(f"   ✅ Conexión OK - {total_productos} productos activos")
        else:
        ...
        try:
            ...
            if embeddings_count > 0:
                print(f"   ✅ {embeddings_count} embeddings disponibles")
            else:
                print("   ⚠️ No hay embeddings - generando...")
                gestor._procesar_todo_el_contenido()
                
                # Verificar después de generar
                count_result = gestor._ejecutar_consulta("SELECT COUNT(*) FROM base_conocimiento_analisis", obtener_datos=True)
                embeddings_count = count_result[0][0] if count_result else 0
                print(f"   ✅ {embeddings_count} embeddings generados")
        ...
        try:
            ...
            if vectorstore:
                ...
                if resultados:
                    print(f"   ✅ Búsqueda funcionando - {len(resultados)} resultados para 'gaming'")
                    ...
                    print("   ⚠️ No se encontraron resultados para 'gaming'")
                
                # Prueba adicional con otro término
                resultados_laptop = gestor.buscar_contenido("laptop", k=2)
                if resultados_laptop:
                    print(f"   ✅ Búsqueda 'laptop': {len(resultados_laptop)} resultados")
        ...
        # Paso 6: Prueba de integración con EntrenamientoFino
        print("\n6️⃣ Probando integración...")
        try:
            ...
            if test_busqueda:
                ...
                print("   ⚠️ Integración parcial - sin resultados de búsqueda")
        ...
        print("✅ Todas las consultas funcionan")
        print("✅ Embeddings disponibles")
        print("✅ Vectorstore operativo")
...
def test_con_entrenamiento_fino():
    """Prueba específica para verificar compatibilidad con EntrenamientoFino"""
    
    print("🔗 PRUEBA DE COMPATIBILIDAD CON ENTRENAMIENTO FINO")
    print("=" * 50)
    
    try:
        # Intentar importar y usar como lo haría entrenamiento_fino.py
        from fix_database_queries import GestorBaseConocimientoFixed as GestorBaseConocimiento
        
        # Simular inicialización como en entrenamiento_fino.py
        print("1️⃣ Simulando inicialización de EntrenamientoFino...")
        gestor_bd = GestorBaseConocimiento()
        print("   ✅ Gestor inicializado correctamente")
        
        # Simular obtención de vectorstore
        print("2️⃣ Obteniendo vectorstore...")
        base_conocimiento = gestor_bd.obtener_vectorstore()
        
        if base_conocimiento:
            print("   ✅ Vectorstore obtenido exitosamente")
            
            # Simular búsqueda como lo haría el sistema
            print("3️⃣ Probando búsqueda semántica...")
            ...
            if resultados:
                ...
                print("   ⚠️ Sin resultados para búsqueda específica")
            
            print("4️⃣ Verificando metadatos...")
            if resultados:
                ...
                for campo in campos_esperados:
                    if campo in metadata:
                        print(f"   ✅ Campo '{campo}': {metadata[campo]}")
                    else:
                        print(f"   ⚠️ Campo '{campo}' faltante")
            
            print("\n🎉 COMPATIBILIDAD VERIFICADA")
            print("✅ El gestor corregido es totalmente compatible")
            print("✅ Puede reemplazar directamente al gestor original")
            
            return True
        else:
            print("   ❌ No se pudo obtener vectorstore")
            return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if comando == "aplicar":
            ...
            print("\n1️⃣ Aplicando parche...")
            if not aplicar_parche():
                print("❌ Falló aplicación de parche")
                sys.exit(1)
            
            print("\n2️⃣ Prueba simple...")
            if not test_simple():
                print("❌ Falló prueba simple")
                sys.exit(1)
            
            print("\n3️⃣ Prueba de compatibilidad...")
            if not test_con_entrenamiento_fino():
                print("❌ Falló prueba de compatibilidad")
                sys.exit(1)
